    ...
    try:
        async for msg in ws:
            if msg.type == web.WSMsgType.TEXT:
                ...
                # 1. Стример просит создать комнату (или ВОССТАНОВИТЬ старую)
                if action == "create":
                    code = data.get("code")
                    my_role = "streamer"
                    
                    if code and code in rooms:
                        # Комната уже существует! Просто возвращаем стримера на место
                        print(f"[{code}] ♻️ Стример переподключился к сети. Комната восстановлена.")
                ...
                # 5. Зритель просит экстренный рестарт
                    ...
                    if code in rooms and rooms[code]["streamer_ws"]:
                        await rooms[code]["streamer_ws"].send_json({"action": "viewer_request_restart"})
                        print(f"[{code}] ⚠️ Зритель сообщил о зависании! Команда на рестарт передана.")
...
app = web.Application()
# Сначала обрабатываем вебсокет, потом корень, а затем динамические ссылки
app.router.add_get('/ws', websocket_handler)
app.router.add_get('/', index_handle)
app.router.add_get('/{code}', index_handle) # <-- НОВОЕ: Ловим ссылки вида /1234
