...
def mss_worker(queue_obj, stop_event, fps, target_size, monitor_idx, resize_mode):
    ...
    try:
        with mss.mss() as sct:
            mon_idx = monitor_idx + 1 if monitor_idx + 1 < len(sct.monitors) else 1
...
class MSSTrack(VideoStreamTrack):
    def __init__(self, fps=60, target_width=1920, target_height=1080, monitor_idx=0, resize_mode="fast", config_state=None):
        super().__init__()
        ...
        self.process = multiprocessing.Process(
            target=mss_worker,
            args=(self.queue, self.stop_event, fps, (target_width, target_height), monitor_idx, resize_mode),
            daemon=True
        )
        self.process.start()
